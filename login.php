...
if (isset($_POST['submit'])) {
     ...
     $client_query = "SELECT * FROM client WHERE email='$email' AND password='$password_s'";
     $client_result = mysqli_query($mysqli, $client_query);
     // Check if the user is an agent
     $agent_query = "SELECT * FROM agent WHERE email='$email' AND password='$password_s'";
     $agent_result = mysqli_query($mysqli, $agent_query);
     // Check if the user is an admin
     $admin_query = "SELECT * FROM manager WHERE email='$email' AND password='$password_s'";
     $admin_result = mysqli_query($mysqli, $admin_query);
     ...
     }else if (mysqli_num_rows($agent_result) == 1) {
...
<body><!-- Start: Navbar Centered Links -->
  <nav class="navbar navbar-light navbar-expand-md fixed-top navbar-shrink py-3" id="mainNav">
    <div class="container"><a class="navbar-brand d-flex align-items-center" href="index.php"><span>ELECTRICAL WEB
          SITE</span></a><button class="navbar-toggler" data-bs-toggle="collapse"><span class="visually-hidden">Toggle
      ...
      <div class="col-md-6 text-center"><img class="img-fluid w-100" src="assets/img/illustrations/login.svg" /></div>
        ...
        ?>
          <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="mb-5"><button class="btn btn-primary shadow" type="submit" name="submit">Log in</button></div>
